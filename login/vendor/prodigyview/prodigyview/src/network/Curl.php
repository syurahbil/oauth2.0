...
class Curl {
	...
	public function sendToken($method, $data = array(), $token) {
		...
		if ($method === 'post') {
			return $this->_postToken($data,$token);
		} else if ($method === 'get') {
			return $this->_getToken($data,$token);
		} else {
			curl_setopt($this->_handler, CURLOPT_CUSTOMREQUEST, strtoupper($method));
			$this->_prepareData($data);
			return $this->_sendCurl($url);
	...
	public function openConnection($url) {
		...
		//curl_setopt($this->_handler, CURLOPT_FOLLOWLOCATION, 1);
		//curl_setopt($this->_handler, CURLOPT_FRESH_CONNECT, TRUE);
	...
	protected function _post($data = array()) {
		...
		curl_setopt($this->_handler, CURLOPT_FOLLOWLOCATION, 1);
		curl_setopt($this->_handler, CURLOPT_CONNECTTIMEOUT, 0); 
		curl_setopt($this->_handler, CURLOPT_TIMEOUT, 90);
		return $this->_sendCurl();
	}
	
	protected function _postToken($data = array(), $token) {
		$this->_prepareData($data);
		$authorization = "Authorization: Bearer ".$token;
		curl_setopt($this->_handler, CURLOPT_POST, 1);
		curl_setopt($this->_handler, CURLOPT_POSTFIELDS, json_encode($data));
		curl_setopt($this->_handler, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($this->_handler, CURLOPT_FOLLOWLOCATION, 1);
		curl_setopt($this->_handler, CURLOPT_SSL_VERIFYPEER, FALSE);
		curl_setopt($this->_handler, CURLOPT_CONNECTTIMEOUT, 0); 
		curl_setopt($this->_handler, CURLOPT_TIMEOUT, 90);
		curl_setopt($this->_handler, CURLOPT_HTTPHEADER, array('Content-Type: application/json' , $authorization ));
	...
	protected function _getToken($data = array(), $token) {
		$url = $this->_url;
		$authorization = "Authorization: Bearer ".$token; 
	...
	protected function _sendCurl() {
		...
		curl_setopt($this->_handler, CURLOPT_FOLLOWLOCATION, TRUE);
		curl_setopt($this->_handler, CURLOPT_FRESH_CONNECT, TRUE);
