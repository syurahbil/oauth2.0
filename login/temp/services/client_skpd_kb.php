...
global $multirule, $datakey, $username, $user_rule, $engineer_id, $session, $host_get_skpd_kb;
...
$no_skpd_kb = CheckString($data['no_skpd_kb']) ?? "";
...
$curl = new Curl($host_get_skpd_kb);
$curl-> send('get', array(
			'no_skpd_kb'=> $no_skpd_kb,
			'no_sspd'=> $no_sspd,
...
function post($data) {
global $multirule, $datakey, $username, $session, $host_post_skpd_kb;
...
{ $sudah_bayar = 0; }
$kurang_bayar = $setoran_seharusnya-$sudah_dibayar;
$denda = str_replace(",","",CheckString($data['denda']));
if (!is_numeric($denda))
{ $denda = 0; }
$jumlah_setoran = $kurang_bayar+$denda;
if (in_array("1",$multirule))
	{
	$allow = encrypt("yes",$datakey);
	}
...
$curl = new Curl($host_post_skpd_kb);
...
//require_once("../../dist/encrypt.php");	
global $multirule, $datakey, $username, $session, $host_get_skpd_kb;
...
$curl = new Curl($host_get_skpd_kb);
$curl-> send('get', array(
			'no_skpd_kb'=> encrypt(CheckString($data['no_skpd_kb']),$datakey),
