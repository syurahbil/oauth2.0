...
$no_skpd_kb = CheckString($data['skpd_kb_status']) ?? "";
...
$query1 = "SELECT * FROM skpd_kb WHERE no_skpd_kb=?";
$cek = $dbcon->query($query1,"$no_skpd_kb");
...
{
			$cek->closeCursor();
			$query1 = "UPDATE skpd_kb SET status=?, status_doc=?, modified_date=NOW(), modified_by=? WHERE no_skpd_kb=?";
			$cek = $dbcon->query($query1,"$status_verifikasi","$status_doc","$username", "$no_skpd_kb");
			if ($cek)
			{
			$cek->closeCursor();
			...
			$cek = $dbcon->query($query1,"$status_verifikasi","$username","$no_skpd_kb");
			
			$cek->closeCursor();
			$query_doc = "INSERT INTO history_status_skpd (no_skpd_kb,status,keterangan,register_date,register_by,flag) VALUES(?,?,'Update Status SKPD-KB = $status_verifikasi dan Status Dokumen= $status_doc dengan keterangan : $keterangan',NOW(),'$username','1')";
			$cek = $dbcon->query($query_doc,"$no_skpd_kb","$status_verifikasi");
			
			$allfield = "UPDATE skpd_kb SET status=?,status_doc=?,  modified_date=NOW(), modified_by=? WHERE no_sspd=? # $status_verifikasi,$status_doc,$username,$no_skpd_kb";
			$allfield = str_replace("'","\'",$allfield);
			
			$query_history = "INSERT INTO history_activity (event, page, username, register_date, remark)
					   VALUES('$allfield','data-skpd-kb','$username',NOW(),'Update Status Verifikasi SKPD-KB')";
...
$cek = $dbcon->query($query1,"$no_skpd_kb","$kode");
$cek->setFetchMode(PDO::FETCH_ASSOC);
$result_cek=$cek->fetchAll();
$rows = $cek->rowCount(); 
if ($rows > 0)
{
			...
			$query1 = "INSERT INTO verifikasi_sspd_skpd_copy (no_reg,jenis,kode,ref_no,keterangan,register_date,modified_date,register_by,modified_by,status,status_dokumen,flag) 
												VALUES (?,'SSPD',?,?,?,NOW(),NOW(),?,?,?,?,?)";
			$cek = $dbcon->query($query1,"$no_skpd_kb",$kode,"$ref_no", "$keterangan", "$username","$username","$status","$status_doc","$flag");
...
$cek = $dbcon->query($query1,"$ref_no","$keterangan","$username", "$status","$status_doc","$flag","$no_skpd_kb","$kode");
			if ($cek)
			{
			 
			$allfield = "UPDATE verifikasi_sspd_skpd SET ref_no=?, keterangan=?, register_date=NOW(), register_by=?,status=?,status_dokumen=?, flag=? WHERE no_reg=? AND kode=? # $ref_no,$keterangan,$username,$status,$status_doc,$flag,$no_skpd_kb,$kode";
			$allfield = str_replace("'","\'",$allfield);
			
			$query_history = "INSERT INTO history_activity (event, page, username, register_date, remark)
					   VALUES('$allfield','check-doc-skpd-kb','$username',NOW(),'Update Status Verifikasi Dokumen SKPD-KB kode $kode')";
			$cek = $dbcon->query($query_history);
...
else
{
			...
			$cek->closeCursor();
			$query1 = "INSERT INTO verifikasi_sspd_skpd (no_reg,jenis,kode,ref_no,keterangan,register_date,register_by,status,status_dokumen,flag) 
												VALUES (?,'SKPD-KB',?,?,?,NOW(),?,?,?,?)";
			$cek = $dbcon->query($query1,"$no_skpd_kb",$kode,"$ref_no", "$keterangan","$username","$status","$status_doc","$flag");
			if ($cek)
			{
			 
			$allfield = "INSERT INTO verifikasi_sspd_skpd (no_reg,jenis,kode,ref_no,keterangan,register_date,register_by,status,status_dokumen,flag) # $ref_no,$keterangan,$username,$status,$status_doc,$flag,$no_sspd,$kode";
			$allfield = str_replace("'","\'",$allfield);
